...
fn intersect_equal() {
    ...
    let iv = IntersectVec::new(FetchVec::new(v0.clone()), FetchVec::new(v1.clone()));
...
fn intersect_many_equal() {
    ...
    let ic = IntersectComplex::new(vecs.into_iter().map(FetchVec::new));
    let ic_values = UnpackVec::new(ic).collect::<Vec<_>>();
    assert_eq!(ic_values, expected);
}
...
fn intersect_same_arrays() {
    ...
    let iv = IntersectVec::new(FetchVec::new(v.clone()), FetchVec::new(v.clone()));
    let iv_values = UnpackVec::new(iv).collect::<Vec<_>>();
    let ic = IntersectComplex::new([FetchVec::new(v.clone()), FetchVec::new(v)].into_iter());
    let ic_values = UnpackVec::new(ic).collect::<Vec<_>>();
...
fn intersect_empty() {
    ...
    let fetch_vecs = [FetchVec::new(v.clone()), FetchVec::new(e.clone())];
    let it = IntersectComplex::new(fetch_vecs.into_iter());
    assert!(UnpackVec::new(it).collect::<Vec<_>>().is_empty());

    let fetch_vecs = [FetchVec::new(e.clone()), FetchVec::new(e.clone())];
    let it = IntersectComplex::new(fetch_vecs.into_iter());
    assert!(UnpackVec::new(it).collect::<Vec<_>>().is_empty());

    let it = IntersectVec::new(FetchVec::new(v), FetchVec::new(e.clone()));
    assert!(UnpackVec::new(it).collect::<Vec<_>>().is_empty());

    let it = IntersectVec::new(FetchVec::new(e.clone()), FetchVec::new(e));
    assert!(UnpackVec::new(it).collect::<Vec<_>>().is_empty());
}
